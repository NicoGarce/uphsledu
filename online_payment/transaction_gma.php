...
<script>
	function getor(refno) {
	   ...
	   if (confirm("If you are sure that this OR number is correct, click OK")==true ) {
		   window.location = "transaction_gma.php?refno="+refno+"&orno="+orno;
	   };
	}
	
	function verifycsv() {
		window.location = "transaction_gma.php?verifycsv=1";
	}
</script>	
  ...
  <tbody>
    <tr>
      <td height="106" colspan="8" align="center" style="font-size: 30px; background-color:blue; color: white"><p>University of Perpetual Help System Laguna<br>
        <font size="4px">GMA Campus</font></p></td>
    </tr>
<form name="frmmain" method="post">
 ...
 <?php 
	...
	$sql = "select * from return_data where txnid like 'UPHG%' and status='Success' ".$add." order by transdate";	   
    $result = mysqli_query($con, $sql);
	$cnt=0;
