...
$shareUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https://' : 'http://') . ($_SERVER['HTTP_HOST'] ?? '') . $base_path . 'uweek/scores.php';
...
.sheet-card{background:var(--card);border:1px solid var(--line);border-radius:16px;overflow:hidden;box-shadow:0 8px 28px rgba(0,0,0,.07);text-align:center;max-width:1100px;margin:0 auto}
...
.sheet-frame{background:#fff;display:flex;justify-content:center;align-items:center;padding:12px}
.sheet-frame iframe{width:100%;max-width:980px;height:80vh;min-height:500px;border:0;display:block;background:#fff;margin:0 auto;border-radius:8px;box-shadow:0 1px 8px rgba(0,0,0,.06)}
...
<div class="uweek-subnav" aria-label="Section navigation">
  <a href="<?php echo $base_path; ?>uweek/scores.php" class="active">Overview</a>
...
<iframe style="width: 100%; height: 80vh; min-height: 500px; border: 0;" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQeJJVxf8aoVBMbOGa0wJAmdn420JC9ouIpAlojMLwtZ7nQ1-vR7e6ZlggMZ8wtGA/pubhtml?gid=791635313&amp;single=true&amp;widget=true&amp;headers=false"></iframe>
    </div>
  </div>
</div>
<?php endif; ?>
