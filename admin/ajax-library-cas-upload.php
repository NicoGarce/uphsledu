<?php
/**
 * Library CAS Thumbnail Upload AJAX Endpoint
 * Accepts one image file (thumbnail) and updates the CAS item's `image` column.
 */
...
if ($cas_id <= 0) { echo json_encode(['success'=>false,'error'=>'cas_id required']); exit; }

$stmt = $pdo->prepare('SELECT slug FROM library_cas WHERE id = :id');
$stmt->execute([':id'=>$cas_id]);
$r = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$r) { echo json_encode(['success'=>false,'error'=>'Item not found']); exit; }
$slug = $r['slug'];
...
if (!empty($imageUrl)) {
    ...
    $up = $pdo->prepare('UPDATE library_cas SET image = :img, updated_at = NOW() WHERE id = :id');
    $up->execute([':img'=>$rel,':id'=>$cas_id]);
...
$dbRelPath = 'assets/images/support-services/college-library/cas/' . $slug . '/' . $final;
$up = $pdo->prepare('UPDATE library_cas SET image = :img, updated_at = NOW() WHERE id = :id');
$up->execute([':img'=>$dbRelPath,':id'=>$cas_id]);
